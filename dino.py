...
class Score:
    ...
    def show(self):
        self.label = self.font.render(f'HI {self.highscore} {self.act}', 1, self.color)
    ...
    def check_hs(self):
        if self.act >= self.highscore:
            self.highscore = self.act
...
class Game:
    ...
    def restart(self):
        self.__init__(hs=self.score.highscore)
